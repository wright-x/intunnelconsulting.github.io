...
PLATEWARE = {
    ...
    "copper_martini": (
        "Served in a polished copper martini glass: a wide tapered conical "
        "bowl at the top, connected by a clearly visible slender copper STEM "
        "(the stem is a mandatory, prominent part of the glass — do not omit "
        "it or show the bowl sitting directly on the table) down to a round "
        "copper foot/base, exactly like a classic martini glass shape but in "
        "polished copper."
    ),
    "rattan_basket": (
    ...
    "chinese_bowl": (
        ...
        "handles on opposite sides (a compact rounded serving bowl, "
        "approximately 19.5cm wide across the handles, 16cm bowl diameter, "
        "5.5cm tall). No sauce well, no dip, no side sauce of any kind — the "
        "dish itself is glossy and saucy, served with nothing extra beside it."
    ),
    "ceramic_cup": (
...
items += [
    ...
    item("Veg Manchurian (Dry)", 195, "crispy vegetable balls tossed with garlic, peppers and a savoury sauce", cat, "chinese_bowl",
         "Glossy dark-glazed vegetable balls with diced bell pepper and spring onion."),
    item("Hakka Noodles", 180, "stir-fried noodles with fresh vegetables, garlic and aromatic seasoning", cat, "chinese_bowl",
         "Glossy stir-fried noodles tangled with julienned carrot, cabbage and bell pepper."),
...
items += [
    ...
    item("Mushroom Tikka", 190, "mushrooms marinated in yogurt and blended Indian spices, grilled to smoky perfection in the tandoor", cat, "round_plain",
         "Whole marinated mushrooms threaded on visible metal skewers (the skewers are mandatory and must be clearly shown, do not plate the mushrooms loose without skewers), char-grilled with visible smoky char marks, thin red onion rings, a lemon wedge and a small metal cup of mint chutney beside."),
    ...
    item("Non-Veg Mixed Platter", 399, "chicken tikka (2pc), chicken malai tikka (2pc), paneer tikka (2pc) and achari paneer tikka (2pc) on one sizzling platter with mint chutney. Serves 2.", cat, "square_plain",
         "A generous assortment of ALL FOUR distinct skewer types clearly visible and separated, two pieces each: char-grilled deep-red chicken tikka, pale creamy-white chicken malai tikka, golden paneer tikka, and paneer achari tikka with a visibly different mustard-yellow pickled-spice crust (the achari paneer must look distinctly different from the plain paneer tikka, not the same color) — all four types fanned out across a square platter with clear gaps between each type so all four are individually identifiable, mint chutney in a small metal cup, thin red onion rings and a lemon wedge, gentle wisps of steam suggesting it just came off the grill."),
...
items += [
    ...
    item("Jeera Aloo", 155, "potatoes sauteed with cumin and aromatic spices", cat, "copper_karahi",
         "Golden baby potatoes tossed with visible cumin seeds and dried red chilli, finished with fresh cilantro, served in the exact same style of small hammered copper karahi on a wooden coaster as the other curries in this set — photorealistic professional food photography, natural texture and lighting, not illustrated or airbrushed-looking."),
...
items += [
    ...
    item("Goan Prawn Curry", 260, "prawns cooked in a coconut-based coastal curry with aromatic spices", cat, "copper_karahi",
         "Plump prawns in a golden-yellow coconut curry, finished with fresh curry leaves. Only the curry in the karahi — no rice, no side of rice, nothing else on the plate."),
]
...
cat = "Drinks"
items += [
    item("Masala Tea", 60, "traditional Indian tea brewed with aromatic spices", cat, "ceramic_cup",
         "Warm reddish-brown spiced milk tea with a light steam rising, a few whole spices (cardamom, cinnamon stick) resting beside the cup. Served in the exact same light speckled ceramic cup and saucer style used for the coffee drinks — not a copper mug."),
